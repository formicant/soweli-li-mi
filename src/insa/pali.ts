...
function panaEKulupuTawa(mamaTawa: NimiPali, suliMa: Lon, lonPali: LonPali, nasin: NasinTawa) {
  ...
  function lukinEKulupuTawa(nanpa: number, lon: Lon): Im.Seq.Keyed<number, Lon> {
    ...
    if (ken) {
      const tawa = panaENanpaTanPali(lukinSin, 'tawa').union(panaENanpaTanPali(lukinSin, 'mi'))
...
function panaENanpaTanPali(
  lukin: Im.Collection<number, Im.Set<LiSeme>> | undefined,
  pali: NimiPali,
) {
  if (lukin !== undefined) {
    return lukin
      .filter(paliMute => paliMute.contains(pali))
      .keySeq()
      .toSet()
  } else {
