...
function panaENasinMusiLinja(linja: ParserOutput<string, NasinMusi>) {
  const nasinLinja = linja.successful ? linja.candidates.map(ken => ken.result) : []
  return Im.Seq(nasinLinja).filterNot(nasinNi =>
...
function liLiliPiNasinMusiAnte(nanpaIjoMuteNi: Im.Set<number>, nanpaIjoMuteAnte: Im.Set<number>) {
  return nanpaIjoMuteNi.size < nanpaIjoMuteAnte.size && nanpaIjoMuteNi.isSubset(nanpaIjoMuteAnte)
}
...
function muteEnInsa<TKulupu, TMute, TInsa>(
  ...
  return apply(seq(pilinMute, rep(seq(pilinInsa, pilinMute))), ([lawa, sijelo]) => [
...
function muteEnSinpin<TKulupu, TMute, TSinpin>(
  ...
  return apply(rep(seq(pilinSinpin, pilinMute)), tu => [
