...
export type KulupuPali = Im.Collection<number, Im.Set<LiSeme>>
export type LonPali = Im.Map<Lon, KulupuPali>
...
export function paliELonPali(maIjo: MaIjo, nasinMusi: readonly NasinMusi[]): LonPali {
  ...
  return Im.Map(
    maIjo.lonIjo.mapEntries(([lon, ijoMute]) => [
      lon,
      ijoMute.map((ijo, nanpa) => panaEPali(lon, ijo, nanpa, maIjo, nasinMusi)),
    ]),
  )
}
